...
module.exports = class GitHandler{
    ...
    //Stash local changes (be sure to add all relative changes before stashing)
    stash(cb){
        if(!this.hasGit) return cb(false);
    ...
    //Similar to stash pop - except that this performs a merge action of stashed changes and forces conflict resolution in favor of the stashed changes
    mergeStash(cb){
        if(!this.hasGit) return cb(false);
        run('git merge --squash --strategy-option=theirs stash', (results) => {
            //After merging stash - drop the stash from the list to better emulate "git stash pop"
            run('git stash drop', (results) => {
                if(!results.success) return cb(false);
                return cb(true);
            })
        })
