...
module.exports = class PackageHandler{
        ...
        this.load((data) => {
            if(data){
                if(typeof options == 'object'){
                    ...
                    mess.status("Attempting to modify package.json");
                    ...
                    if(madeChange){
                        this.save(data, (success) => {
                            if(success){
                                var successfulLock = this.lock();
                                if(successfulLock){
                                    mess.success('Finished making changes to package.json file.');
                                    return cb(true);
                                }else{
                                    return cb(false);
                                }
    ...
    lock(){
        ...
        try{
            execSync('npm shrinkwrap', {timeout: 20000});
        }catch(e){
