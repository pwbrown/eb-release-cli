...
function createRelease(envName, cb){
    mess.load("Creating release branch");
    perf.start("Creating release branch");
    git.getCurrentBranch((startingBranch) => {
        ...
        git.changes((changes) => {
            ...
            git.hasBranch(releaseBranch, (hasBranch) => {
                if(hasBranch)
                    mess.status("Moving to Release Branch");
                else
                    mess.status("Creating Release Branch");
                git.checkout(releaseBranch, () => {
                    mess.status("Committing all changes" + (ignored? ", and " + ignored.length + " ignored change(s), " : ""));
                    git.add(null, ignored, () => {
                        git.commit(envName, () => {
                            var shouldKeep = config.shouldKeep();
                            ...
                            git.push(releaseBranch, () => {
                                mess.stopLoad(true);
                                perf.end("Creating release branch");
...
function deployApplication(envName, shouldDeploy, cb){
    if(shouldDeploy){
        ...
        mess.load("Deploying release branch to Elastic Beanstalk");
