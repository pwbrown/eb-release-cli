...
exports.runAsync = function(command, options, cb){
    ...
    exec(command, {timeout: options.timeout, encoding: 'utf8', env: combineEnv(options.env)}, function(err, stdout, stderr){
        if(err) return cb({success: false});
        else return cb({success: true, stdout: stdout});
    })
}

function combineEnv(envVars){
    if(!envVars) return null; //Null defaults to process.env;
    var env = JSON.parse(JSON.stringify(process.env)); //Copy existing environment variables
